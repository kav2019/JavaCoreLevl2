    ...
    private void messageListener(DataInputStream dataInputStream) throws IOException {
        while (true) {
            ...
            if (newMessage.equals("/exit")) {
                ...
                String messageFill = newMessage.substring(3);
                int messageIndex = messageFill.indexOf(" ");
                String nick = messageFill.substring(0, messageIndex);
                String massage = messageFill.substring(messageIndex);
                server.sendMessageTo(client, nick, massage);
